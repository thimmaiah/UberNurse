...
Scenario Outline: Pricing Request
  ...
  Examples:
    ...
    |verified=true|role=Admin |role=Care Giver;speciality=Generalist|{hour:8, min:10}   |{hour:14}   | {carer_weekday:9, care_home_weekday:10}  |{care_home_base:58.33, care_home_total_amount:72}    | 54 |    
    ...
    |verified=true|role=Admin |role=Nurse;speciality=Mental Health  |{hour:20, min:10}  |{hour:23}   |{carer_weekday:9, carer_weeknight:10, care_home_weekday:10, care_home_weeknight:15}  |{care_home_base:45, care_home_total_amount:54}    | 30 |    
